...
export default function Timetable() {
  ...
  return (
    <Layout title="Timetable">
              ...
              <tbody>
                {grid.map((row, i) => (
                  <tr key={i} className={i === 0 ? "bg-ink text-paper" : "border-t border-ink/10"}>
                    {row.map((cell, j) => (
                      <td key={j} className={`px-2.5 py-1.5 ${i === 0 ? "font-black" : "font-semibold text-ink/80"}`}>
                        {cell || "-"}
                      </td>
                    ))}
                  </tr>
      ...
      {mappings.length > 0 && (
        <div className="mt-3 overflow-x-auto rounded-lg border border-ink/10 bg-white p-2.5 shadow-soft">
          <h2 className="text-xs font-black uppercase tracking-wider text-ink/40">Subject - Faculty</h2>
          <div className="mt-2 space-y-1.5">
            {mappings.map((m, i) => (
              <div key={i} className="flex items-center justify-between gap-3">
                <span className="text-xs font-bold text-ink/80">{m.subject}</span>
                <span className="text-xs font-semibold text-ink/50">{m.faculty}</span>
              </div>
            ))}
          </div>
        </div>
